...
static void on_inotify_monitor_cb_impl(int event_type, int argc, const char *args[], void *p_data)
{
    ...
    debug_print("inotify in monitor --> %s\n", path.data());
    CriticalSection lock(&p_file_monitor->stap_spinlock);
    ...
    while (true) {
        ...
        {
            /* stap_cache something error */
        }

        /* NOTE: event is unusable from now */
    ...
    // clear timeout monitor_cache
    if(p_file_monitor->monitor_cache.size() > 10000)
    {
        time_t current_time = event->time;
        std::multimap<std::string, file_event_t *>::iterator itr = p_file_monitor->monitor_cache.begin();
        while (itr != p_file_monitor->monitor_cache.end())
        {
            if (itr->second->time + DEFAULT_TTL_SPAN < current_time) {
                file_event_free(itr->second);
                p_file_monitor->monitor_cache.erase(itr++);
            } else {
                itr++;
            ...
            uint32_t option_count, const char *(*pp_options)[2])
{
    ...
    pthread_spin_init(&p_file_monitor->stap_spinlock, PTHREAD_PROCESS_PRIVATE);
    pthread_rwlock_init(&p_file_monitor->path_lock, NULL);
...
PUBLIC_LIB void destroy_file_monitor(file_monitor_t *p_file_monitor)
{
    ...
    if (p_file_monitor)
    {
        ...
        pthread_spin_destroy(&p_file_monitor->stap_spinlock);
        pthread_rwlock_destroy(&p_file_monitor->path_lock);
