...
char **str_split(const char *str, const char sep, int *num_toks)
{
    ...
    int max_sub = 1;
    for(const char *cur=str; cur<str_end; cur++)
    {
        if(sep == *cur)
            max_sub++;
    }
//    printf("there is [%d] sub in [%s]\n", max_sub, str);

    char **ret = (char **)calloc(max_sub, sizeof(char *));
    if(!ret)
        return NULL;

    int cur_sub = 0;
    for(const char *cur=str, *sub=str; cur <= str_end; cur++)
    {
        if(sep == *sub || '\0' == *sub)
        {
            ret[cur_sub++] = strdup("");
        ...
        else if(sep == *cur || '\0' == *cur)
        {
            ret[cur_sub++] = strndup(sub, cur-sub);
            sub = cur+1;
        }
    }
    if(num_toks)
