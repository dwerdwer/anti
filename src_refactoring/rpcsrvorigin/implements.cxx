...
class RpcServer : public IRpcServer
{
    public:
        ...
        {
            const char *p_buffer = p_message->Get_StringValue(4);
            if (p_buffer != NULL && strlen(p_buffer) > 0)
            {
                printf("########## + receive message from sdk ##########\n");
                printf("data:%s\n", p_buffer);
        ...
        int postData(uint64_t tag, const char *buffer, int lenght)
        {
            printf("##########  %s ##########\n", buffer);
            if (buffer == NULL)
                return 0;
            void *ptr_data = mMapConn[tag];
            if (ptr_data)
            {
                IRpcConnection *ptr_conn = (IRpcConnection *)ptr_data;
                IRpcMessage *ptr_message = NewInstance_IRpcMessage(2, 3);
                ptr_message->Add_StringValue(4, buffer);
                ptr_message->Add_IntValue(3, 495);
    ...
    private:
        ...
        std::map<uint16_t, void *> mMapConn;
        void *m_p_event_loop;
        void *m_p_user_data;
};
    ...
    if (strcmp(cmd, "antivirus_result") == 0)
    {
        ...
        if (p_module)
        {
            if (p_module->p_UsrData)
            {
                ...
                p_rpcserver->postData(data_tag, data_buffer, data_length);
            }
        }
    }
