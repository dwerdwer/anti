        ...
        void *p_in, uint32_t in_len)
{
    ...
    if (false == p_protocol_context->is_pending_stop)
    {
        ...
        {
            if(p_state->sp_shared_state)
            {
                p_state->p_conn_params = p_callback->connection_create(&(p_state->client_addr), p_state->sp_shared_state);
                p_state->sp_shared_state->p_conn_params = p_state->p_conn_params;
            }
            else
            {
                p_state->p_conn_params = p_callback->connection_create(&(p_state->client_addr), p_connection);
            }
...
void do_send(IN void* p_evloop, void* p_token, const char *p_data, uint32_t data_length)
{
    lws_server_evloop_t *p_loop = (lws_server_evloop_t*)p_evloop;
    message_t *p_message = create_message(p_data, data_length);
    if (NULL == p_loop)
        return ;
    if (p_loop->protocol_context.p_thread_pool == NULL) //sync
    {
        lws_connection_t *p_connection = (lws_connection_t*)p_token;
        if (NULL != p_connection)
        {
            lws_connection_state_t *p_state = (lws_connection_state_t*)lws_wsi_user(p_connection);
            if (NULL != p_message &&
                    NULL != p_state &&
                 0 == store_sending_message(NULL, p_state, p_message))
            {
                lws_callback_on_writable(p_connection);
            }
            else if (NULL != p_message)
            ...
            {
                callback_base *p_callback = p_loop->protocol_context.p_callback;
	            p_callback->notify(p_state->p_conn_params,
	               SERVER_NOTIFY_RECEIVE_SELF_REQUEST_ERROR, &(p_state->client_addr),
	               p_data, data_length);
    ...
    {
        conn_shared_state_t *sp_shared_state = (conn_shared_state_t*)p_token;
        if (NULL != p_message &&
                NULL != sp_shared_state)
