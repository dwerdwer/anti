...
extern void test_server_lock(struct lws *wsi, void *p_user, int care);
extern void test_server_unlock(struct lws *wsi, void *p_user, int care);
	       ...
	       char *buf, int len, enum lws_spa_fileupload_states state)
{
	...
	case LWS_UFS_CONTENT:
		if (len) {
			...
			n = (int)write((int)pss->post_fd, buf, (size_t)len);
		  ...
		  void *in, size_t len)
{
	...
	case LWS_CALLBACK_HTTP_BODY:
		...
		if (!pss->spa) {
			pss->spa = lws_spa_create(wsi, param_names,
					(int)(ARRAY_SIZE(param_names)), 1024,
					file_upload_cb, pss);
			if (!pss->spa)
				return -1;
	...
	case LWS_CALLBACK_HTTP_BODY_COMPLETION:
		...
		for (n = 0; n < (int)(ARRAY_SIZE(param_names)); n++)
			p += lws_snprintf((char *)p, (size_t)(end - p),
				    "<tr><td><b>%s</b></td><td>%d</td><td>%s</td></tr>",
				    param_names[n],
				    lws_spa_get_length(pss->spa, n),
				    lws_spa_get_string(pss->spa, n));

		p += lws_snprintf((char *)p, (size_t)(end - p), "</table><br><b>filename:</b> %s, <b>length</b> %ld",
				pss->filename, pss->file_length);

		p += lws_snprintf((char *)p, (size_t)(end - p), "</body></html>");
		pss->result_len = (int)(p - (unsigned char *)(pss->result + LWS_PRE));
		...
		if (lws_add_http_header_by_token(wsi, WSI_TOKEN_HTTP_CONTENT_TYPE,
			...
			return 1;
		if (lws_add_http_header_content_length(wsi, (lws_filepos_t)(pss->result_len), &p, end))
			return 1;
		if (lws_finalize_http_header(wsi, &p, end))
			return 1;
		...
		n = lws_write(wsi, (unsigned char *)pss->result + LWS_PRE,
			      (size_t)(pss->result_len), LWS_WRITE_HTTP);
		if (n < 0)
			return 1;
		goto try_to_reuse;
	...
	case LWS_CALLBACK_HTTP_WRITEABLE:
		...
		do {
			...
			n = lws_vfs_file_read(pss->fop_fd,
					       &amount, buffer + LWS_PRE, (lws_filepos_t)n);
			 ...
			 */
			m = lws_write(wsi, buffer + LWS_PRE, (size_t)n, LWS_WRITE_HTTP);
			if (m < 0) {
...
bail:
	...
	case LWS_CALLBACK_ESTABLISHED_CLIENT_HTTP: {
		...
		ctlen = (char)lws_hdr_copy(wsi, ctype, sizeof(ctype), WSI_TOKEN_HTTP_CONTENT_TYPE);
		if (ctlen > 0) {
			if (lws_add_http_header_by_token(lws_get_parent(wsi),
				WSI_TOKEN_HTTP_CONTENT_TYPE,
	...
	case LWS_CALLBACK_LOCK_POLL:
		 ...
		 */
		test_server_lock(wsi, user, (int)len);
		break;
	...
	case LWS_CALLBACK_UNLOCK_POLL:
		 ...
		 * if protecting wsi lifecycle change, len == 1
		 */
		test_server_unlock(wsi, user, (int)len);
		break;
		...
		if (crl_path[0]) {
			...
			if (n != 1) {
				...
				lwsl_err("LWS_CALLBACK_OPENSSL_LOAD_EXTRA_SERVER_VERIFY_CERTS: SSL error: %s (%d)\n", 
                        ERR_error_string((long unsigned int)n, errbuf), n);
				return 1;
			}
		}
		break;
