...
void *create_file_digest_cache(const char *p_db_path)
{
    ...
    if((p_sqlite = create_sqlite_connect
        (p_db_path, get_element_cb, p_cache_data->tab_name)) == NULL)
    {
    ...
    else
    {
        ...
        snprintf(cmd_buf, MAX_CMD_LEN, "DROP TABLE IF EXISTS '%s';", p_cache_data->tab_name);

        if (sqlite_command(p_sqlite, cmd_buf) != 0)
            return NULL;

        snprintf(cmd_buf, MAX_CMD_LEN, "CREATE TABLE '%s' (%s);",
...
static int access_sqlite(cache_data_t *p_cache, file_digest_t *p_file_digest)
{
    ...
    if (sqlite_command(p_cache->p_sqlite, cmd_buf) != 0)
        return -1;
...
static int judge_eleinfo_state(cache_data_t *p_cache, const char *p_file_path) 
{
    ...
    if (get_element_by_str(p_cache->p_sqlite, &p_file_digest, "file_path", p_file_path) != 0)
        return 0;
...
file_digest_t *get_cache_ele(void *p_cache_data, int id)
{
    ...
    if (get_element_by_id(p_cache->p_sqlite, &p_file_digest, id) != 0)
    {
        return NULL;
    }
...
file_digest_t *get_file_digest_by_md5(void *p_cache_data, const char *md5_str)
{
    ...
    if (get_element_by_str(p_cache->p_sqlite, &p_file_digest, "md5", md5_str) != 0)
    {
        return NULL;
    }
    return p_file_digest;
...
int delete_file_digest_by_md5(void *p_cache_data, const char *md5_str)
{
    ...
    if (delete_element_by_str(p_cache->p_sqlite, "md5", md5_str) != 0)
        return -1;
...
int delete_file_digest_by_path(void *p_cache_data, const char *p_file_path)
{
    ...
    if (delete_element_by_str(p_cache->p_sqlite, "file_path", p_file_path) != 0)
        return -1;
...
int get_file_digest_top_id(void *p_cache_data)
{
    ...
    if ((top_id = get_top_element_id(p_cache->p_sqlite)) == -1)
        return -1;
